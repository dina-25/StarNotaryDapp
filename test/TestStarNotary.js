...
it('lets userSeller get the fund after the sale', async() => {
    ...
    let valueBefore = Number(balanceOfSellerBeforeTransaction) + Number(starPrice);
...
it('lets userBuyer buy a star and decreases its balance in ether', async() => {
    ...
    let accBalance = Number(balanceOfBuyerBeforeTransaction) - Number(balanceOfBuyerAfterTransaction);
...
it('lets 2 users exchange stars', async() => {
    ...
    assert.equal(starOwner1, user1);
    assert.equal(starOwner2, user2);
});
...
it('lets a user transfer a star', async() => {
    ...
    await instance.transferStar(user, starId);
